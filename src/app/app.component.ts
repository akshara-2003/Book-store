...
export class AppComponent {
  ...
  total:number =0;
  cartItems :cartItem [] =[];
  // itemQuantity:Array<quantityInterface> = []
  counter :number=1;
  insertProduct(product:Product){
    ...
    // this.total = this.total+Math.floor(product.price)*(this.cartItems.quantity);
  }
  removeItem(id:number){
      let itemIndex = this.cartItems.findIndex(obj => obj.product.id ===id)
    //   this.total = this.total - Math.floor(this.cartItems[itemIndex].product.price);
      this.cartItems.splice(itemIndex,1);
     
  }
  totalPrice(){
    return this.cartItems.reduce((total,item)=>total+item.product.price.value*item.quantity,0);
  }
